<template>
  <div>
    <h2 id="page-heading" data-cy="SupplierHeading">
      <span v-text="$t('workshopApp.supplier.home.title')" id="supplier-heading">Suppliers</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('workshopApp.supplier.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'SupplierCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('workshopApp.supplier.home.createLabel')"> Create a new Supplier </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && suppliers && suppliers.length === 0">
      <span v-text="$t('workshopApp.supplier.home.notFound')">No suppliers found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('workshopApp.supplier.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('email')">
              <span v-text="$t('workshopApp.supplier.email')">Email</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'email'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('phone')">
              <span v-text="$t('workshopApp.supplier.phone')">Phone</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'phone'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('mobilePone')">
              <span v-text="$t('workshopApp.supplier.mobilePone')">Mobile Pone</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'mobilePone'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="workshopApp.supplier.delete.question" data-cy="supplierDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-supplier-heading" v-text="$t('workshopApp.supplier.delete.question', { id: removeId })">
