<template>
  <div>
    <h2 id="page-heading" data-cy="LocationHeading">
      <span v-text="$t('workshopApp.location.home.title')" id="location-heading">Locations</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('workshopApp.location.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'LocationCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('workshopApp.location.home.createLabel')"> Create a new Location </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && locations && locations.length === 0">
      <span v-text="$t('workshopApp.location.home.notFound')">No locations found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('workshopApp.location.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('streetAddress')">
              <span v-text="$t('workshopApp.location.streetAddress')">Street Address</span>
            ...
            <th scope="row" v-on:click="changeOrder('city')">
              <span v-text="$t('workshopApp.location.city')">City</span>
            ...
            <th scope="row" v-on:click="changeOrder('stateProvince')">
              <span v-text="$t('workshopApp.location.stateProvince')">State Province</span>
            ...
            <th scope="row" v-on:click="changeOrder('lang')">
              <span v-text="$t('workshopApp.location.lang')">Lang</span>
            ...
            <th scope="row" v-on:click="changeOrder('lat')">
              <span v-text="$t('workshopApp.location.lat')">Lat</span>
            ...
            <th scope="row" v-on:click="changeOrder('country.id')">
              <span v-text="$t('workshopApp.location.country')">Country</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="workshopApp.location.delete.question" data-cy="locationDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-location-heading" v-text="$t('workshopApp.location.delete.question', { id: removeId })">
