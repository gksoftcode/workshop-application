<template>
  <div>
    <h2 id="page-heading" data-cy="DepartmentHeading">
      <span v-text="$t('workshopApp.department.home.title')" id="department-heading">Departments</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('workshopApp.department.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'DepartmentCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('workshopApp.department.home.createLabel')"> Create a new Department </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && departments && departments.length === 0">
      <span v-text="$t('workshopApp.department.home.notFound')">No departments found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="departments">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('departmentName')">
              <span v-text="$t('workshopApp.department.departmentName')">Department Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'departmentName'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('parentDepartment.id')">
              <span v-text="$t('workshopApp.department.parentDepartment')">Parent Department</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'parentDepartment.id'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="department in departments" :key="department.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="department.parentDepartment">
                <router-link :to="{ name: 'DepartmentView', params: { departmentId: department.parentDepartment.id } }">{{
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="workshopApp.department.delete.question" data-cy="departmentDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-department-heading" v-text="$t('workshopApp.department.delete.question', { id: removeId })">
