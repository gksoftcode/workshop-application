<template>
  <div>
    <h2 id="page-heading" data-cy="CountryHeading">
      <span v-text="$t('workshopApp.country.home.title')" id="country-heading">Countries</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('workshopApp.country.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CountryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('workshopApp.country.home.createLabel')"> Create a new Country </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && countries && countries.length === 0">
      <span v-text="$t('workshopApp.country.home.notFound')">No countries found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('workshopApp.country.name')">Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="workshopApp.country.delete.question" data-cy="countryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-country-heading" v-text="$t('workshopApp.country.delete.question', { id: removeId })">
