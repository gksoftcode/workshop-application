<template>
  <div>
    <h2 id="page-heading" data-cy="StatusHeading">
      <span v-text="$t('workshopApp.status.home.title')" id="status-heading">Statuses</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('workshopApp.status.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'StatusCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('workshopApp.status.home.createLabel')"> Create a new Status </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && statuses && statuses.length === 0">
      <span v-text="$t('workshopApp.status.home.notFound')">No statuses found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('workshopApp.status.name')">Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="workshopApp.status.delete.question" data-cy="statusDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-status-heading" v-text="$t('workshopApp.status.delete.question', { id: removeId })">
